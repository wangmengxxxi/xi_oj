...
| 工具名 | 功能 | 调用链路 | 仅限 Agent |
|---|---|---|---|
| `query_question_info` | 按关键词/ID 查题 | `QuestionService.getByKeyword()` | OJChatAgent |
| `judge_user_code` | 提交代码判题 | `AiJudgeService.submitCode()` | OJChatAgent |
| `query_user_wrong_question` | 查用户错题记录 | `WrongQuestionService.getByUserAndQuestion()` | OJChatAgent |

### 11.2 AI 判题隔离机制（AiJudgeService）
...
| 优化项 | 描述 | 优先级 |
|---|---|---|
| 错题复习提醒 | 基于 `nextReviewTime` 的定时提醒推送 | P1 |
| 学习看板 | 错题趋势、复习进度、AI 使用统计可视化 | P2 |
| 知识库增量同步 | 题目新增/修改时增量更新向量，替代全量重建 | P1 |
| 多模型支持 | 配置中心支持多模型切换（如 GPT-4、Claude 等） | P2 |
...
5. 业务创新：自动错题收集、艾宾浩斯复习机制、ReAct 工具调用、相似题推荐。
